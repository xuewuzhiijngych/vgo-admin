...
import { getList, deleteInfo, exportInfo, addInfo, editInfo, changeInfoStatus } from "@/api/modules/article";
import ArticleFormDrawer from "@/views/cms/article/FormDrawer.vue";
...
// 切换用户状态
const changeStatus = async (row: Article.ResList) => {
  await useHandleData(changeInfoStatus, { id: row.id, status: row.status == 1 ? 2 : 1 }, `切换【${row.title}】状态`);
  proTable.value?.getTableList();
};

// 表格配置项
const columns = reactive<ColumnProps<Article.ResList>[]>([
  {
    prop: "id",
    label: "ID"
  },
...
// 打开 drawer(新增、查看、编辑)
const drawerRef = ref<InstanceType<typeof ArticleFormDrawer> | null>(null);
const openDrawer = (title: string, row: Partial<Article.ResList> = {}) => {
  const params = {
    title,
    isView: title === "查看",
    row: { ...row },
...
// 删除信息
const deleteAccount = async (params: Article.ResList) => {
